    ...
    while true; do
        ...
        if [ "$HB_CODE" = "200" ]; then
            ...
            if [ -n "$HB_PROXY" ] && [ -n "$HB_LAN" ] && [ -n "${WG_IFACE:-}" ]; then
                # Recreate if missing or proxy_subnet changed.
                # nft rule format: "ip daddr <proxy> dnat ..." — proxy appears before dnat.
                if ! nft list chain ip ctrlnat prerouting 2>/dev/null | grep -qF "daddr $HB_PROXY" && \
